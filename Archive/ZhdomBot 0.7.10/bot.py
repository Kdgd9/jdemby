...
__version__ = "0.7.9" # Версия бота (увеличена)
        ...
        for chat_id in active_chat_ids:
            ...
            if time_since_last >= interval:
                ...
                try:
                    ...
                    logger.error(f"Chat {chat_id}: Timeout error getting random message from {model_key}")
                    settings['last_sent'] = current_time + 60 # Попробуем снова через минуту
                except Exception as e:
                    logger.exception(f"Chat {chat_id}: Error processing random message: {e}")
                    settings['last_sent'] = current_time + 60 # Попробуем снова через минуту
                    await asyncio.sleep(1) # Небольшая общая пауза при ошибке
...
async def process_question(
    ...
    try:
        ...
        await safe_send_message(message, f"⚠️ Произошла ошибка при обработке вашего запроса: {escape_html(str(e))}", reply_to_message=True, parse_mode=None, bot_instance=message.bot) # Передаем bot
...
async def cmd_model(message: types.Message, state: FSMContext):
    ...
    if not available_models:
         await safe_send_message(message, "❌ Нет доступных моделей для выбора.", reply_to_message=False, parse_mode=None, bot_instance=message.bot) # Передаем bot
         return
...
async def process_model_selection(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Handling model selection from user {message.from_user.id if message.from_user else 'unknown'} in chat {message.chat.id}")
    # Проверяем, что сообщение текстовое
    if not message.text:
        await safe_send_message(message, "❌ Пожалуйста, используйте кнопки для выбора.", reply_to_message=False, reply_markup=ReplyKeyboardRemove(), parse_mode=None, bot_instance=message.bot) # Передаем bot
    ...
    if selected_model_key in storage.get_available_models():
        ...
        await safe_send_message(
            message,
            f"✅ Модель успешно изменена на <b>{escape_html(selected_model_key)}</b>!", # Экранируем имя модели
            reply_to_message=False,
...
async def cmd_image(message: types.Message, command: CommandObject, bot: Bot): # Добавляем bot как аргумент
    ...
    try:
        ...
        image_model_key = storage.get_model_key(message.chat.id)
        image_provider_config = PROVIDERS.get(image_model_key)
        IMAGE_PROVIDERS = {"Bing", "OpenaiChat", "GooglePalm", "Gemini", "DeepInfra", "Replicate", "Pollinations"}
        is_suitable_provider = image_provider_config and image_provider_config.get("provider") in IMAGE_PROVIDERS

        if not is_suitable_provider:
             found_suitable = False
             for key, config in PROVIDERS.items():
                 if config.get("provider") in IMAGE_PROVIDERS:
                     image_model_key = key
                     image_provider_config = PROVIDERS.get(image_model_key) # Получаем конфиг найденной модели
             ...
             if not found_suitable:
                 ...
                 await safe_send_message(message, "❌ Не найдена подходящая модель для генерации изображений в конфигурации.", reply_to_message=True, parse_mode=None, bot_instance=bot) # Передаем bot
                 return
        ...
        if not image_provider_name or not image_model_identifier or image_provider_name not in PROVIDER_MAP:
             ...
             if status_msg: await status_msg.delete()
             await safe_send_message(message, f"⚠️ Ошибка конфигурации для модели изображений '{escape_html(image_model_key)}'.", reply_to_message=True, parse_mode=None, bot_instance=bot) # Передаем bot
             return
        ...
        if hasattr(g4f, 'image') and hasattr(g4f.image, 'create_async') and callable(g4f.image.create_async):
            logger.info("g4f.image.create_async found and is callable. Attempting direct image generation.")
            try:
        ...
        if not image_generation_successful:
            ...
            try:
                ...
                chat_response = await g4f.ChatCompletion.create_async(
                    model=image_model_identifier, # Use the same model if it's suitable for chat
                    provider=image_provider_class,
                    messages=[{"role": "user", "content": image_generation_prompt}],
                    timeout=180
                )
                response_str = str(chat_response)
                # Search for URLs in the response
        ...
        if image_urls:
            ...
            logger.warning(f"No image URL found in response for prompt: {prompt[:50]}. Response: {response[:200] if isinstance(response, str) else response}")
            await safe_send_message(message, "❌ Не удалось получить URL изображения от модели. Попробуйте другой запрос или модель.", reply_to_message=True, parse_mode=None, bot_instance=bot) # Передаем bot
        ...
        await safe_send_message(message, f"⚠️ Ошибка генерации изображения: {escape_html(str(e))}", reply_to_message=True, parse_mode=None, bot_instance=bot) # Передаем bot
...
async def handle_group_text_message(message: types.Message):
    ...
    if message.text and not (message.text.startswith('/') or (BOT_USERNAME and message.text.lower().startswith(f'@{BOT_USERNAME.lower()}'))):
        ...
        chat_id = message.chat.id
        user_id = message.from_user.id
        is_group = True # Это групповой чат
